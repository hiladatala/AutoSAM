...
class ImageLoader(torch.utils.data.Dataset):
    ...
    def preload_all_slices(self):
        file_path = "/media/cilab/DATA/Hila/Data/Projects/AutoSAM/preloaded_images.pkl"
        if os.path.exists(file_path):
            # If the file exists, load the images
            ...
            for file_path in self.paths:
                ...
                mask = self.loader(os.path.join(self.masks_root, mask_path), is_mask=False)
                mask = (mask == 6)
                ...
                # Resize the image and mask to 256x256x1  20
                img = zoom(img, (50 / img.shape[0], 50/ img.shape[1], 50/img.shape[2]))
                mask = zoom(mask, (50/ mask.shape[0], 50/ mask.shape[1], 50/mask.shape[2]))
                mask_values = np.unique(mask)
                ...
                for i in range(50):
                    ...
                    #img_slice = torch.tensor(img_slice).cuda()
                    #mask_slice = torch.tensor(mask_slice).cuda()
                    ...
                    #img_slice_scaled = img_slice * 1 / 3
                    #img_slice = torch.cat([img_slice_scaled.unsqueeze(-1)] * 3, dim=-1)
            ...
            with open("/media/cilab/DATA/Hila/Data/Projects/AutoSAM/preloaded_images.pkl", "wb") as f:
